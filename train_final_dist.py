...
def train(opt):
    ...
    while(True):
        ...
        if utils.is_main_process() and (iteration + 1) % 1e+4 == 0:
            torch.save(
...
if __name__ == '__main__':
    ...
    opt = get_args()
    if 'pickle' in opt.character: # Load a pickle file that contains the most used Korean languages
        with open(opt.character, 'rb') as fr:   
            korean_dict = pickle.load(fr)
        opt.character = ''.join(sorted(korean_dict))
    ...
    if opt.sensitive:
        opt.character = string.printable[:-6]  # Case sensitive
